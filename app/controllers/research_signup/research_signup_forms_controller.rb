module ResearchSignup
  class ResearchSignupFormsController < ApplicationController
    before_action :ensure_eitc_estimate_present, only: %i[edit update]

    helper_method :current_eitc_estimate, :step_count, :current_step, :current_path, :next_path, :form_name
    ...
    def current_path(params = nil)
      research_path(self.class.to_param, params)
    end

    def next_path(params = {})
      next_step = form_navigation.next
      if next_step
        research_path(next_step.to_param, params)
      end
    end
    ...
    def form_navigation
      @form_navigation ||= ResearchFormNavigation.new(self)
    end
